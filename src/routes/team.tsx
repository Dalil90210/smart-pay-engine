  ...
  head: () => ({
    meta: [
      { title: "Team — Smart Pay Engine" },
      {
        name: "description",
        content:
          "Invite teammates, manage roles and set approval thresholds for outbound Smart Pay Engine transfers.",
      },
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Role-based access with multi-party approvals on large payments.
        </p>
      </div>
          ...
          {[
            {
              tier: "Under $5,000",
              rule: "Auto-approved",
              who: "Any Approver or Admin",
              tone: "bg-success/10 text-success border-success/30",
            },
            {
              tier: "$5,000 – $25,000",
              rule: "1 Approver required",
              who: "Approver or Admin",
              tone: "bg-cyan/10 text-cyan border-cyan/30",
            },
            {
              tier: "$25,000+",
              rule: "Dual approval",
            ...
            <div key={p.tier} className="rounded-lg border border-border bg-background/50 p-3">
              <div
                className={`inline-flex rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${p.tone}`}
              >
                {p.tier}
              </div>
            ...
            <div key={a.id} className="flex flex-wrap items-center gap-3 py-3">
              <div className="min-w-0 flex-1">
                ...
                <div className="text-xs text-muted-foreground">
                  Requested by {a.requestedBy} · {a.route}
                </div>
              </div>
              {a.status === "pending" ? (
                <div className="flex items-center gap-2">
        ...
        <div className="divide-y divide-border">
          {members.map((m) => (
            <div key={m.id} className="flex items-center gap-3 py-3">
              ...
              <Badge variant="outline" className={roleStyle[m.role]}>
                {m.role}
              </Badge>
            </div>
          ))}
